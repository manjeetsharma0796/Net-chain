...
**ALL WR1-WR10 done, deployed (v1.0.5, netchain.vercel.app), verified live 2026-07-12:** full
settle proven on-ledger (SME net payer −45k → Carol +15k / Investor +30k, real updateId confirmed on
the 5N validator); over-cap settle rejected on-ledger and the UI shows the failure (no green, no tx, no
Lighthouse, balances unchanged); privacy-check shows real projections + a real CONTRACT_NOT_FOUND on a
foreign id; audit lists real updateIds with Lighthouse links; parties show Carol/Investor/SME/Dave.
...
| T65 | P1 | DAML | DONE: Maker-checker (four-eyes) treasury-cap governance (v1.0.4 SCU, pkg `8ae58d7c…768cbb`). New `TreasuryPolicyProposal`: party (maker) proposes `newCap` on its own policy, operator (checker) `ApproveCapChange` atomically archives old + issues new; `RejectCapChange`/`WithdrawCapChange`. Neither changes a cap alone; operator still can't unilaterally raise a cap. Backend propose/approve/reject/list + routes + wrappers; MCP tools (16 total); policy-page UI (propose + approve/reject). `test_cap_change_maker_checker` (CI green). Chose maker-checker over party-controlled per SOX/Basel/treasury segregation-of-duties norms. Deduped duplicate policies + made all readers use the latest policy so the change reads back. Verified live: 200k→propose→approve→300k→restored | Jishnu | ✅ | T48 |
| T66 | P1 | FE | DONE: `Re-verify live` (and the activity feed + net-position recovery) broke once the ledger passed 200 updates: `/v2/updates/flats` caps at 200 elements and ERRORS beyond it, so the full-history scan silently failed ("live ledger unreachable"). `verifyUpdate` now fetches the exact update by id via `/v2/updates/update-by-id` (O(1), immune to the cap); `updateHistory` widens a tail window until it would exceed 200 then uses the widest that fit. Verified live: real updateId → confirmed:true + effectiveAt, bogus → false, activity feed back to 8 events | Jishnu | ✅ | T23 |
| T64 | P2 | FE | DONE: Automated netting (no manual operator trigger). `/api/cron/net` (GET, `CRON_SECRET`-gated) runs one funding-bounded cycle + Settle, then `reseedOpenLedger()` self-heals the demo back to clean OPEN so the public URL is never left drained. Wired to a **free Vercel Cron** (`vercel.json`, daily `0 0 * * *`; sub-daily needs Pro). reseedOpenLedger parallelizes archives+creates (was a timeout as sequential). Also `daml/reseed.sh` for one-command manual reset. Verified live: trigger → 200, real settle updateId, nets +15/+30/-45 sum 0, balances self-healed to 100k; 401 without the secret. Every on-ledger guarantee still holds, so automating the trigger changes nothing about correctness | Jishnu | ✅ | T48 |
...
> **Redeploy status (2026-07-12):** the live package is now **v1.0.4**, id
> `8ae58d7ccd979213f10c78a328da0f7edfa3614060a4cf7250216837a7768cbb` (T65 maker-checker cap
> governance: `TreasuryPolicyProposal`). SCU upgrade chain:
